...
def pdf_page_to_markdown(pdf_path: str, page_num: int) -> str:
    """
    将PDF单页转换为Markdown格式（用于SDL_agent项目集成）
        ...
        for block in blocks:
            if block["type"] == 0:  # 文本块
                for line in block["lines"]:
                    ...
                    page_elements.append(final_text)

        doc.close()
        return '\n'.join(page_elements)

    except Exception as e:
        print(f"PDF页面转Markdown失败: {e}")
        return ""


def detect_complex_content(text: str) -> bool:
    """
    检测文本中是否包含复杂内容（化学式、实验数据、图表引用等）
    如果包含这些内容，建议使用Vision API而不是纯文本提取
    ...
    if not text or len(text.strip()) < 100:
        return True  # 文本为空或太短，可能是图片页，使用Vision

    # 检测化学式特征（需要更严格的匹配）
    chemical_patterns = [
        r'[A-Z][a-z]?[₀-₉]+',  # 下标数字
        r'[A-Z][a-z]?\d{2,}',  # 化学式如 FAPbI3（数字>=2）
        r'\b[A-Z][a-z]?[A-Z][a-z]?\d+',  # 复合化学式如 CH3NH3
    ]
    ...
    if chemical_count >= 2:  # 至少匹配2个化学式特征
        return True
