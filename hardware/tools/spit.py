...
@register_tool(
    ...
    description="滴液",
    params={
        "spit_pos": {"type": "int", "description": "需要滴液的目标基底的位置,[x,y,z]", "required": True, "default": [0,0,0]},
...
def spit(spit_pos: Tuple[int, int, int], tip: int, vol: int) -> str:
    ...
    try:
        ...
        if client.is_connected:
            ...
            return f"滴液机已保存:{tip}号泵向x={spit_pos[0]},y={spit_pos[1]},z={spit_pos[2]}位置滴{vol}ul溶液"
        else:
            ...
            if connect_state:
                ...
                return f"滴液机滴液步骤保存失败"
    except Exception as e:
        return f"滴液机滴液步骤保存失败: {str(e)}"
